...
1. Log in to your domain registrar and open the nameserver settings for the domain.
...
On the server, copy `.env.example` to `.env` at the project root and set `CLOUDFLARE_TUNNEL_TOKEN` to the tunnel token from the previous step.
